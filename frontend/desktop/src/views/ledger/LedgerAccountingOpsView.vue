<template>
  <div class="acct-ops">
    ...
    <section v-else-if="section === 'attach'" class="detail-card">
      ...
      <p class="field-hint">可为附件标注辅助核算：部门、项目、往来（F41）；审计包导出时将一并包含（F48）。</p>
      <div class="form-row">
        <label>事件 Seq</label>
...
import AttachmentAuxFields from '../../components/ledger/AttachmentAuxFields.vue'
...
async function saveAttachmentAux(attachId) {
  auxSaving.value = attachId
  try {
    await api.patchAttachmentAuxiliary(lid(), attachId, draftAux[attachId] || {})
