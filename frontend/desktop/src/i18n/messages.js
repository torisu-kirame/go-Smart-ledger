export const messages = {
  zh: {
    dashboard: {
      title: '系统概览',
      refresh: '刷新',
      cardGateway: '网关',
      cardChain: '链节点',
      cardHeight: '区块高度',
      cardLedgers: '账本数',
      ok: '正常',
      quickTitle: '常用功能',
      guideTitle: '使用指引',
      guide1: '在「账本管理」创建私人或多人账本；多人账本可配置审批、加密与记账模板。',
      guide2: '进入账本详情可记一笔、封账锚定、校验完整性；多人账本可邀请成员（「好友」→ 邀请）。',
      guide3: '「Excel 导入」批量入账；「备份 / 恢复」导出或还原账本快照。',
      guide4: '「记账模板」管理字段结构；「团队」将多人账本与好友编组协作。',
      guide5: '「链浏览器」查看区块与上链队列；若记账上链失败，可在概览 / 队列页重试。',
      openChain: '打开链浏览器 →',
      queueAlertPending: '上链队列：{pending} 条待重试',
      queueAlertBoth: '上链队列：{pending} 条待重试，{failed} 条失败',
      loadFail: '加载失败',
    },
    layout: {
      settings: '设置',
      logout: '退出',
      chainLink: '链浏览器',
      nav: {
        home: '概览',
        ledgers: '账本管理',
        templates: '记账模板',
        import: 'Excel 导入',
        backup: '备份 / 恢复',
        friends: '好友',
        teams: '团队',
        invites: '账本邀请',
        chain: '链浏览器',
      },
    },
    settings: {
      title: '设置',
      ai: {
        title: '离线 AI（OpenClaw）',
        enabled: '启用本地 AI 助手集成',
        provider: '模型提供方',
        baseUrl: 'API 地址（聊天与向量）',
        chatModel: '对话模型',
        embedModel: '向量模型',
        apiKey: 'API Key（Ollama 可填 ollama）',
        gateway: 'OpenClaw Gateway 地址',
        copyConfig: '复制 OpenClaw 配置片段',
        copied: '已复制到剪贴板，可粘贴到 openclaw/openclaw.json',
        copyFail: '复制失败',
      },
      theme: {
        title: '主题设置',
        mode: '昼夜模式',
        light: '浅色',
        dark: '深色',
        accent: '主题色',
        language: '界面语言',
        langZh: '简体中文',
        langEn: 'English',
      },
      personal: {
        title: '个人设置',
        loading: '加载中…',
        userId: '用户 ID',
        username: '登录名',
        createdAt: '注册时间',
        nickname: '昵称',
        nicknamePh: '展示名称，最多 32 字',
        saveNickname: '保存昵称',
        saving: '保存中…',
        deleteTitle: '注销账号',
        password: '密码',
        deleteBtn: '永久注销账号',
        deleting: '注销中…',
        avatarTitle: '点击或拖拽上传头像',
      },
      loadFail: '加载失败',
      saveOk: '昵称已更新',
      saveFail: '保存失败',
      avatarOk: '头像已更新',
      avatarFail: '上传失败',
      avatarSize: '图片不能超过 2MB',
      deleteConfirm: '确定永久注销？此操作不可撤销。',
      deleteFail: '注销失败',
    },
    chain: {
      title: 'MiniLedger 链浏览器',
      tabOfficial: '区块浏览器',
      tabOverview: '概览 / 队列',
      nodeOnline: '在线',
      nodeOffline: '离线',
      cardNode: '链节点',
      cardRole: '共识角色',
      cardQueue: '待上链队列',
      height: '高度',
      pendingRetry: '待重试',
      failed: '已失败',
      consensus: '共识状态',
      recentBlocks: '最近区块',
      refresh: '刷新',
      colHeight: '高度',
      colHash: '哈希',
      colTxs: '交易数',
      colTime: '时间',
      noBlocks: '暂无区块数据',
      queueTitle: '上链重试队列',
      noQueue: '当前没有待重试的上链任务',
      colLabel: '标签',
      colLedger: '账本',
      colStatus: '状态',
      colAttempts: '尝试',
      colError: '错误',
      retry: '立即重试',
      loadFail: '加载链状态失败',
      retryFail: '重试失败',
      statusPending: '待处理',
      statusRetrying: '重试中',
      statusFailed: '失败',
      statusDone: '完成',
      iframeTitle: 'MiniLedger 区块浏览器',
    },
  },
  en: {
    dashboard: {
      title: 'Overview',
      refresh: 'Refresh',
      cardGateway: 'Gateway',
      cardChain: 'Chain node',
      cardHeight: 'Block height',
      cardLedgers: 'Ledgers',
      ok: 'OK',
      quickTitle: 'Shortcuts',
      guideTitle: 'Getting started',
      guide1: 'Create private or multi-user ledgers under Ledgers; configure approval, encryption, and entry templates.',
      guide2: 'Open a ledger to post entries, anchor, and verify; invite members via Friends for multi-user ledgers.',
      guide3: 'Use Excel Import for bulk entries; Backup / Restore for snapshots.',
      guide4: 'Manage field schemas in Entry Templates; use Teams to group collaborators.',
      guide5: 'Open Chain Explorer for blocks and the submit queue; retry failed submissions from Overview or the queue tab.',
      openChain: 'Open chain explorer →',
      queueAlertPending: 'Submit queue: {pending} pending retry',
      queueAlertBoth: 'Submit queue: {pending} pending, {failed} failed',
      loadFail: 'Failed to load',
    },
    layout: {
      settings: 'Settings',
      logout: 'Log out',
      chainLink: 'Chain Explorer',
      nav: {
        home: 'Overview',
        ledgers: 'Ledgers',
        templates: 'Entry Templates',
        import: 'Excel Import',
        backup: 'Backup / Restore',
        friends: 'Friends',
        teams: 'Teams',
        invites: 'Invites',
        chain: 'Chain Explorer',
      },
    },
    settings: {
      title: 'Settings',
      ai: {
        title: 'Offline AI (OpenClaw)',
        enabled: 'Enable local AI assistant integration',
        provider: 'Model provider',
        baseUrl: 'API base URL (chat + embeddings)',
        chatModel: 'Chat model',
        embedModel: 'Embedding model',
        apiKey: 'API key (use "ollama" for Ollama)',
        gateway: 'OpenClaw Gateway URL',
        copyConfig: 'Copy OpenClaw config snippet',
        copied: 'Copied — paste into openclaw/openclaw.json',
        copyFail: 'Copy failed',
      },
      theme: {
        title: 'Appearance',
        mode: 'Color mode',
        light: 'Light',
        dark: 'Dark',
        accent: 'Accent',
        language: 'Language',
        langZh: '简体中文',
        langEn: 'English',
      },
      personal: {
        title: 'Profile',
        loading: 'Loading…',
        userId: 'User ID',
        username: 'Username',
        createdAt: 'Registered',
        nickname: 'Nickname',
        nicknamePh: 'Display name, up to 32 chars',
        saveNickname: 'Save nickname',
        saving: 'Saving…',
        deleteTitle: 'Delete account',
        password: 'Password',
        deleteBtn: 'Delete account permanently',
        deleting: 'Deleting…',
        avatarTitle: 'Click or drag to upload avatar',
      },
      loadFail: 'Failed to load',
      saveOk: 'Nickname updated',
      saveFail: 'Save failed',
      avatarOk: 'Avatar updated',
      avatarFail: 'Upload failed',
      avatarSize: 'Image must be under 2MB',
      deleteConfirm: 'Delete account permanently? This cannot be undone.',
      deleteFail: 'Delete failed',
    },
    chain: {
      title: 'MiniLedger Chain Explorer',
      tabOfficial: 'Block Explorer',
      tabOverview: 'Overview / Queue',
      nodeOnline: 'Online',
      nodeOffline: 'Offline',
      cardNode: 'Node',
      cardRole: 'Consensus role',
      cardQueue: 'Submit queue',
      height: 'Height',
      pendingRetry: 'pending',
      failed: 'failed',
      consensus: 'Consensus',
      recentBlocks: 'Recent blocks',
      refresh: 'Refresh',
      colHeight: 'Height',
      colHash: 'Hash',
      colTxs: 'Txs',
      colTime: 'Time',
      noBlocks: 'No blocks',
      queueTitle: 'Chain retry queue',
      noQueue: 'No pending submissions',
      colLabel: 'Label',
      colLedger: 'Ledger',
      colStatus: 'Status',
      colAttempts: 'Attempts',
      colError: 'Error',
      retry: 'Retry now',
      loadFail: 'Failed to load chain status',
      retryFail: 'Retry failed',
      statusPending: 'pending',
      statusRetrying: 'retrying',
      statusFailed: 'failed',
      statusDone: 'done',
      iframeTitle: 'MiniLedger Block Explorer',
    },
  },
}

function getByPath(obj, path) {
  return path.split('.').reduce((o, k) => (o && o[k] != null ? o[k] : undefined), obj)
}

export function translate(locale, key) {
  const pack = messages[locale] || messages.zh
  const val = getByPath(pack, key)
  return val != null ? val : key
}
